...
public class PresentationRepositoryTest extends BaseTestWithDBAccess {
    ...
    public void testFindByCreatorIdentifier_typicalCase_shouldReturnsCorrectly() {
        List<Presentation> actualList = presentationRepository.findByCreatorIdentifier("1");
        Assert.assertEquals(2, actualList.size());

        actualList = presentationRepository.findByCreatorIdentifier("2");
        Assert.assertEquals(1, actualList.size());

        actualList = presentationRepository.findByCreatorIdentifier("3");
